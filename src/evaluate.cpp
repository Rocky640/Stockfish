...
namespace {
  ...
  Score Evaluation<T>::evaluate_threats() {
    ...
    Bitboard b, b1, weak, defended, stronglyProtected, safeThreats;
    Score score = SCORE_ZERO;
    ...
    if (defended | weak)
    {
        b1 = b = (defended | weak) & attackedBy[Us][KNIGHT];
        while (b)
            score += ThreatByMinor[type_of(pos.piece_on(pop_lsb(&b)))];

        b1 |= b = (defended | weak) & attackedBy[Us][BISHOP];
        while (b)
            score += ThreatByMinor[type_of(pos.piece_on(pop_lsb(&b)))];

        b1 |= b = (pos.pieces(Them, QUEEN) | weak) & attackedBy[Us][ROOK];
        while (b)
            score += ThreatByRook[type_of(pos.piece_on(pop_lsb(&b)))];

        b1 &= ~(pos.pieces(PAWN) | attackedBy[Us][PAWN]);
        while (b1)
            score += ThreatByRank * (int)relative_rank(Them, pop_lsb(&b1));
