...
namespace {
  ...
  const Bitboard BishopCenter = (Rank4BB | Rank5BB) & CenterFiles;
    ...
    while ((s = *pl++) != SQ_NONE)
    {
        ...
        if (Pt == BISHOP || Pt == KNIGHT)
        {
            ...
            if (Pt == BISHOP)
            {
                ...
                // Bonus for bishop if it can "see" 2 empty squares in the "BishopCenter"
                if (  !(attackedBy[Them][PAWN] & s)
                    && (more_than_one(BishopCenter & (attacks_bb<BISHOP>(s, pos.pieces(PAWN)) | s))))
