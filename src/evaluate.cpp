...
#define S(mg, eg) make_score(mg, eg)
  ...
  // ThreatByMinor/ByRook[strong/weak/hanging][attacked PieceType] contains bonuses according to
  // which piece type attacks which one. Attacks on lesser pieces which are
  // pawn-defended are not considered.
  // Special PIECE_TYPE_NB = 0 is used for threat on a blocked pawn, and is added to the PAWN bonus

  Score ThreatByMinor[3][PIECE_TYPE_NB] = {
    { S( 9,-14), S(-2, -4), S(32, 46), S( 60, 51), S( 71, 135), S(35, 110) },//strongly defended
    { S(-7, 19), S( 9, 22), S(47, 55), S( 84, 62), S( 81, 115), S(74, 122) },//slightly defended (weak)
    { S(51, 65), S(54, 55), S(92, 63), S(120, 75), S(113, 152), S(99, 142) } //hanging
  };

  Score ThreatByRook[3][PIECE_TYPE_NB] = {
    { S( 0, -8), S(-2,  6), S(10,  0), S(  2, -2), S( 7,  3), S(26, 50) }, //strongly defended
    { S( 3, 43), S( 5, 14), S(49, 88), S( 48, 80), S(27, 44), S(53, 45) }, //slightly defended (weak)
    { S(58, 50), S(51, 43), S(99,103), S(112,102), S(50, 60), S(86, 69) }  //hanging
  };

   // ThreatByRank[strong/weak/hanging]
  Score ThreatByRank[3] = {  S( 10,  1), S( 16,  1), S( 18,  4)};

  // ThreatByKing[strong/weak/hanging]
  Score ThreatByKing[3] = {  S( -7, -2), S( 45, 86), S( 83, 109)};
...
#undef S
  ...
  Score Evaluation<T>::threats() const {
    ...
    if (defended | weak)
    {
        ...
        {
            Square s = pop_lsb(&b);
            score += ThreatByMinor[bool(weak & s)][type_of(pos.piece_on(s))];
            if (type_of(pos.piece_on(s)) != PAWN)
                score += ThreatByRank[bool(weak & s)] * (int)relative_rank(Them, s);
            else if (blocked & s)
                score += ThreatByMinor[bool(weak & s)][0];
        }
        ...
        {
            Square s = pop_lsb(&b);
            score += ThreatByRook[bool(weak & s)][type_of(pos.piece_on(s))];
            if (type_of(pos.piece_on(s)) != PAWN)
                score += ThreatByRank[bool(weak & s)] * (int)relative_rank(Them, s);
            else if (blocked & s)
                score += ThreatByRook[bool(weak & s)][0];
        }
