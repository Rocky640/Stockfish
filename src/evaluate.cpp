...
namespace {
  ...
  Score Evaluation<T>::evaluate_threats() {
    ...
    // Consider only the squares which are not completely unsafe, or which can
    // lead to possible breaks on the king side.
    b &=  (~attackedBy[Them][PAWN]
        & (attackedBy[Us][ALL_PIECES] | ~attackedBy[Them][ALL_PIECES]))
        | file_bb(ksq) | adjacent_files_bb(file_of(ksq));
