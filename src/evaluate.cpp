...
#define S(mg, eg) make_score(mg, eg)
  ...
  // Outpost[knight/bishop][no pawn support/pawn support/pawn supported pawn support] contains bonuses for minor
  // pieces if they occupy or can reach an outpost square, bigger if that
  // square is supported by a pawn or by a safe pawn
  constexpr Score Outpost[][3] = {
    { S(22, 6), S(36,12), S(46,18)}, // Knight
    { S( 9, 2), S(15, 5), S(15, 5)}  // Bishop
  };
        ...
        if (Pt == BISHOP || Pt == KNIGHT)
        {
            ...
            if (bb & s)
                score += Outpost[Pt == BISHOP][  bool(attackedBy[Us][PAWN] & s)
                                               + bool(pe->supp_attacks(Us) & s)] * 2;

            else if (bb &= b & ~pos.pieces(Us))
                score += Outpost[Pt == BISHOP][  bool(attackedBy[Us][PAWN] & bb)
                                               + bool(pe->supp_attacks(Us) & bb)];
