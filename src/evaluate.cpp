...
namespace {
  ...
  TUNE(SetRange(0, 400), a1, a2, a3, a4);
  TUNE(SetRange(500, 1000), a5);
  TUNE(SetRange(0, 50), a6);
  TUNE(SetRange(-100, 100), a7);
  ...
  Score evaluate_king(const Position& pos, const EvalInfo& ei) {
    ...
    if (ei.kingAttackersCount[Them])
    {
        ...
        b1 = ei.attackedBy2[Them] & ei.attackedBy[Us][KING];
        ...
        kingDanger =  std::min(807, ei.kingAttackersCount[Them] * ei.kingAttackersWeight[Them])
                    + a1 * popcount(undefended)
                    + a2 * popcount(b1)
                    + a3 * popcount(b2) + a4 * !!ei.pinnedPieces[Us]
                    - a5 * !pos.count<QUEEN>(Them)
                    - a6 * mg_value(score) / 32 - a7;
