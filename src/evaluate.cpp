...
namespace {
  ...
  Score Evaluation<T>::evaluate_initiative(Value eg) {
    ...
    Bitboard b       =  attackedBy[WHITE][ALL_PIECES] & mobilityArea[WHITE]
                      & attackedBy[BLACK][ALL_PIECES] & mobilityArea[BLACK];
    bool bothFlanks  = (pos.pieces(PAWN) & QueenSide) && (pos.pieces(PAWN) & KingSide);

    // Compute the initiative bonus for the attacking side
    int initiative =   8 * (pe->pawn_asymmetry() + kingDistance - popcount(b) - 12)
                    + 12 * pos.count<PAWN>()
                    + 16 * bothFlanks;
