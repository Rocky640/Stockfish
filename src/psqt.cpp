...
constexpr Score Bonus[][RANK_NB][int(FILE_NB) / 2] = {
  ...
  { // King
   { S(273,  2), S(326, 39), S(261, 79), S(184, 66) },
   { S(280, 55), S(300, 99), S(222,127), S(184,123) },
  ...
  { // Assymetry adjustment for King on queen side squares on first 2 ranks
   { S(-4,-6), S(24,-6), S(22, -9), S(17, 26) },
   { S(12, 0), S(2 ,-5), S(23, 22), S( 8, 10) }
  }
};
...
void init() {
  ...
  for (Piece pc = W_PAWN; pc <= W_KING; ++pc)
  {
      ...
      for (Square s = SQ_A1; s <= SQ_H8; ++s)
      {
          ...
          if (pc == W_KING && rank_of(s) < RANK_3 && file_of(s) < FILE_E)
              psq[pc][s] += Bonus[pc + 1][rank_of(s)][f];
          psq[~pc][~s] = -psq[pc][s];
      }
  }
}
