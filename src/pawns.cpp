...
namespace {
  ...
  // Backward pawn penalty by opposed, and by safe sentry:
  // we score a bit more if one of the bwstopper is not attacked by a pawn.
  const Score Backward[2][2] = {
    { S(49, 26), S(60, 37) },   //not opposed
    { S(34, 12), S(45, 23) } }; //opposed
  ...
  Score evaluate(const Position& pos, Pawns::Entry* e) {
    ...
    Bitboard b, neighbours, stoppers, bwstoppers, doubled, supported, phalanx;
    Square s;
    bool isolated, opposed, lever, connected;
    ...
    while ((s = *pl++) != SQ_NONE)
    {
        ...
        else
        {
            ...
            // If we have an enemy pawn (a bwstopper) in the same or next rank, 
            // the pawn is backward because it is opposed or it can be captured before 
            // making a phalanx with closest neighbour.
            bwstoppers = (b | shift_bb<Up>(b & adjacent_files_bb(f))) & stoppers;
        }
        ...
        if (isolated)
            ...
            score -= Backward[opposed][!!(bwstoppers & ~e->pawnAttacks[Us])];
