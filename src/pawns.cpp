  ...
  Score evaluate(const Position& pos, Pawns::Entry* e) {
    ...
    while ((s = *pl++) != SQ_NONE)
    {
        ...
        else
        {
            ...
            // (that is : if the side neighbour move up, the pawn will become a backward)
            // But this true only if the phalanx is a strict duo (not a trio), and the side phalanx is not doubled
            // and finally, the sentry is not under attack and the pawn is on rank 2 or 3
            if (phalanx && (more_than_one((phalanx | shift_bb<DELTA_E>(phalanx) | shift_bb<DELTA_W>(phalanx)) & ourPawns & ~SquareBB[s])
