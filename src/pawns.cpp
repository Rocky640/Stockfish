...
namespace {
  ...
  Score evaluate(const Position& pos, Pawns::Entry* e) {
    ...
    Bitboard b, neighbours, stoppers, doubled, support, phalanx, opposed;
    ...
    e->freePawnAttacksSpan[Us] = e->pawnAttacksSpan[Us] = e->passedPawns[Us] = 0;
    ...
    while ((s = *pl++) != SQ_NONE)
    {
        ...
        if (opposed)
            e->freePawnAttacksSpan[Us] |= pawn_attack_span(Us, s) ^ pawn_attack_span(Us, frontmost_sq(Them, opposed));
        else
            e->freePawnAttacksSpan[Us] |= pawn_attack_span(Us, s);
    }
